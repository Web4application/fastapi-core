...
_Then, to start your application by default:
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync
Changing the UV_LINK_MODE silences warnings about not being able to link files since the cache and sync target are on separate file systems.
```
